...
The human view follows [Gareji UI language v0](ui-language-v0.md). It presents the target and overall state first, aligns the stable status and layer labels under `System`, uses `Recent activity` for bounded checkpoint evidence, and emits at most one concrete `Next:` instruction. The presentation is deterministic plain text and does not depend on color or an interactive terminal. `--json` is unchanged by human-presentation revisions.
