...
`--dry-run` validates all local inputs and reports the intended layers without copying Core or invoking mutating Core or Codex commands. `--json` returns the same bounded report for automation.
...
One successful setup performs these operations in order:
